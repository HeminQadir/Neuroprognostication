...
def load_data(data_folder, patient_id, device, train=True):
    ...
    # Load EEG recording.    
    eeg_channels = ['F3', 'P3', 'F4', 'P4']  #['Fp1', 'F3', 'C3', 'P3', 'F7', 'T3', 'T5', 'O1', 'Fz', 'Cz', 'Pz', 'Fp2', 'F4', 'C4', 'P4', 'F8', 'T4', 'T6', 'O2'] # #['F3', 'P3', 'F4', 'P4'] #
    group = 'EEG'

    size = 30000
    sampling_frequency = 100
    bipolar_data = torch.zeros((2, size), dtype=torch.float32)
            ...
            if os.path.exists(recording_location + '.hea'):
                ...
                if length >= five_min_recording:
                    ...
                    if all(channel in channels for channel in eeg_channels):
                        ...
                        bipolar_data = torch.zeros((2, data.shape[1]), dtype=torch.float32)
                        bipolar_data = bipolar_data.to(device)
